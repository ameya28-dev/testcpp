...
std::ostream& operator<<(std::ostream& stream, const std::unordered_map<int, std::string>& map) {
    ...
    stream << "std::unordered_map<int, std::string>{{" << map.begin()->first << "," << std::quoted(map.begin()->second)
           << "}";
    std::for_each(std::next(map.begin()), map.end(), [&stream](const std::pair<int, std::string>& pair) {
...
std::ostream& operator<<(std::ostream& stream, const std::unordered_map<int, int>& map) {
    ...
    std::for_each(std::next(map.begin()), map.end(),
        [&stream](const std::pair<int, int>& pair) { stream << ",{" << pair.first << "," << pair.second << "}"; });
    return stream << "}";
}
...
int main() {
    ...
    std::set<int> s;
    std::transform(
        intersect.begin(), intersect.end(), std::inserter(s, std::begin(s)), [](int a) { return a * a + 2; });
