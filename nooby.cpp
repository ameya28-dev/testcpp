...
void train_model(const std::vector<int>& data, auto& model) {
    ...
    std::for_each(data.cbegin(), data.cend(), [&model](const auto& x) { model.update(x); }); // not an improvement
}
...
auto get_values_return_anon_struct(const int n) {
    struct V {
        int x, y;
    };
...
void function_evaluation_order_not_guaranteed() {
    ...
    int i            = 0;
    const auto inc_i = [&i]() -> int { return ++i; };
...
void constructing_a_unique_ptr_directly() {
    ...
    f(std::unique_ptr<int>(new int{}), std::unique_ptr<float>(new float{})); // could leak prior to C++17
...
struct Pizza {
    Pizza(float diameter, std::vector<std::string> toppings) : m_diameter{diameter}, m_toppings{std::move(toppings)} {}
