...
auto find_mismatch(const std::vector<int>& v, const std::vector<int>& v2) {
    if (v.size() != v2.size()) {
        std::cout << "v and v2 are different size => mismatch found\n";
        return v2.end();
    }
    if (auto res = std::mismatch(v.cbegin(), v.cend(), v2.cbegin()); res.first == v.cend() && res.second == v2.cend()) {
        std::cout << "v and v2 have no mismatch\n";
        return v2.end();
    } else {
        auto& [first, second] = res;
        std::cout << "v and v2 has a mismatch\n"
                  << "First position:\t" << std::distance(v.cbegin(), first) << "Value:\t" << *first << "\n"
                  << "Second position:\t" << std::distance(v2.cbegin(), second) << "Value:\t" << *second << "\n";
        return second;
    }
}

auto is_positive = [](const int a) { return a % 2 == 0; };
...
int main() {
    ...
    if (auto res = std::find_if(v.cbegin(), v.cend(), is_positive); res != v.cend()) {
    ...
    std::cout << std::count_if(v.begin(), v.end(), is_prime) << std::endl;
    {
        ...
        std::string s2;
        std::copy_if(s1.cbegin(), s1.cend(), std::back_inserter(s2), [](const char a) { return (a > 64 && a < 91); });
        std::cout << s2 << '\n';
